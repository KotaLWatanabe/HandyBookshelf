...
object ExternalApiError {
  ...
  final case class HttpError(status: Status, message: String, responseBody: Option[String] = None)
      extends ExternalApiError
  ...
  private def buildRequest[T: Encoder](
    ...
    IO.delay {
      ...
      val requestWithAuth = config.apiKey.fold(requestWithHeaders) { apiKey =>
        requestWithHeaders.putHeaders(
          org.http4s.headers.Authorization(org.http4s.Credentials.Token(org.http4s.AuthScheme.Bearer, apiKey))
        )
      }
