...
/**
 * リポジトリパッケージ Event Sourcingアーキテクチャにおけるリポジトリインターフェースを提供
 *
 * 主要なリポジトリ:
...
package object repositories:
   ...
   * 永続化エラー
   */
  final case class PersistenceError(override val message: String, cause: Option[Throwable] = None)
      extends RepositoryError

  /**
   * 同時実行制御エラー
   */
  final case class ConcurrencyError(aggregateId: String, errorMessage: String, cause: Option[Throwable] = None)
      extends RepositoryError:
