...
/**
 * 汎用的な集約リポジトリの基底トレイト Event Sourcingにおける集約の永続化と復元を抽象化
 */
trait AggregateRepository[A <: AggregateRoot[A, E], E <: DomainEvent]:
  ...
  /**
   * 集約のスナップショットを保存する（オプション） 長いイベント履歴を持つ集約のパフォーマンス最適化に使用
   * @param aggregate
   *   スナップショットを保存する集約
   * @return
   *   保存成功を示すIO
