...
object BookIdentifier:
  ...
  /** ISBN による識別（書籍に推奨）
    *
    * ISBNは国際的に一意な書籍識別子であり、 最も信頼性の高い重複チェックが可能。
    * ISBN-10とISBN-13は同一視される。
    */
  ...
  /** arXiv ID による識別（論文に推奨）
    *
    * arXivは学術論文のプレプリントサーバーで、
    * 各論文に一意のIDが付与される。
    * バージョン番号（v1, v2等）は無視して比較する。
    */
  ...
  /** DOI による識別（学術論文・文献に推奨）
    *
    * DOI（Digital Object Identifier）は学術出版物の
    * 永続的識別子。大文字小文字は区別しない。
    */
  ...
  /** タイトルによる識別（フォールバック）
    *
    * 上記の識別子がない場合に使用。
    * 同一タイトルの異なる文献が存在しうるため、
    * 厳密な一意性は保証されない。ユーザーへの警告表示に使用。
    */
...
/** NormalizedIdentifier - 正規化された識別子
  *
  * 異なる形式の識別子（ISBN-10/13、arXivバージョンあり/なし、DOI大文字/小文字）
  * を統一的に比較するための正規化された形式。
  *
